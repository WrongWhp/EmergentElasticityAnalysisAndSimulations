...
for cur_key  = mouse_cell_ratemap_megastruct.keys();
    ...
    for run_number = 1:length(cur_ratemap_struct_list)
        ...
        if(isfield(rate_map_filter, 'mean_rate_bounds'))
           should_use_ratemap_struct =  should_use_ratemap_struct && IsBetween(cur_ratemap_struct.open_field_scores.mean_rate, rate_map_filter.mean_rate_bounds(1), rate_map_filter.mean_rate_bounds(2));
        end
