...
%k_from_conf = sqrt(qchisq(conf,r)); % r is the number of dimensions (degrees of freedom)

k_from_conf = 1; % r is the number of dimensions (degrees of freedom)
...
if r==3 & c==3
  ...
  h1=plot3(x0+k_from_conf*x,y0+k_from_conf*y,z0+k_from_conf*z,prop.style);hold on
  [y,z,x] = getpoints(Cyz,prop.clip);
  h2=plot3(x0+k_from_conf*x,y0+k_from_conf*y,z0+k_from_conf*z,prop.style);hold on
  [z,x,y] = getpoints(Czx,prop.clip);
  h3=plot3(x0+k_from_conf*x,y0+k_from_conf*y,z0+k_from_conf*z,prop.style);hold on
  ...
  X(:) = scale*(k_from_conf*XYZ(:,1)+x0);
  Y(:) = scale*(k_from_conf*XYZ(:,2)+y0);
  Z(:) = scale*(k_from_conf*XYZ(:,3)+z0);
  h4=surf(X,Y,Z);
  colormap yellow
  alpha(0.8)
  camlight
...
elseif r==2 & c==2
  ...
  h1=plot(scale*(x0+k_from_conf*x),scale*(y0+k_from_conf*y),prop.style);
  set(h1,'zdata',z+1)
  if nargout
    h=h1;
  end
