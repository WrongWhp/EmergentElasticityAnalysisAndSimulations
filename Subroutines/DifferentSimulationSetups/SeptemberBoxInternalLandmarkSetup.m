...
for landmark_cell_radius = .5
    for cur_force_mult =  2 * .5 * 1 * 2 * .5 * (.25)
        for wall_force_mult = .25 * [1];
            for rh_wave_vec = (2* pi/5) * (1.2:.2:2.4)
                ...
                sysparams.folder_path = sprintf('../Outputs/SeptBoxInternalLandmark/Graining%d/Radius%f/Wavevec%f/ArchOutputStrength%f/WallMult%f/',  sysparams.graining, landmark_cell_radius, rh_wave_vec,  cur_force_mult, wall_force_mult);
                ...
                if(1)
                    ...
                    if(1)
                        ...
                        last_landmark_cell = landmark_cell_list{i};
                        last_landmark_cell.mask = (( abs(system.mesh.x - 2*system.mesh.y + 4) < 1) .*( abs(system.mesh.x + system.mesh.y - 12)<4))>0;
                        last_landmark_cell.strength = last_landmark_cell.strength * 94;
                                                
                        landmark_cell_list{i + 1} = last_landmark_cell;
                    end
                ...
                [a, b] = WallLocalWeighting(sysparams, system, landmark_cell_list)                
