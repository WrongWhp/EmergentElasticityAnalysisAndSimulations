...
for graining = [2]
    ...
    for krupic_ish_boundaries = [ 1 0]
        for landmark_cell_radius = .5
            for cur_force_mult =  .5
                for wall_force_mult = 1 * [2] * 1;
                    for rh_rot_angle = [7] * (pi/180 * 1);
                        for rh_wave_vec = fliplr((2* pi) * (1.0./[3 4 5 6]))
                            %[1.0 *(krupic_ish_boundaries) + (5/3) *(1- krupic_ish_boundaries)];
                            ...
                            sysparams.n_runs = 20;
                            sysparams.steps_per_pixel = 2000; %Average number of times the mouse will step foot in each pixel
...
%                            sysparams.steps_per_pixel = 200; %Average number of times the mouse will step foot in each pixel
                            ...
                            if(krupic_ish_boundaries)
                                sysparams.folder_path = sprintf('../Outputs/FebTrapSetup/Krupicish/Graining%d/Radius%fAspratio%f/RHWavevec%f/Angle%f/ArchOutputStrength%f/WallMult%f/',  sysparams.graining, landmark_cell_radius, sysparams.ellipse_asp_ratio, rh_wave_vec, sysparams.rh_rot_angle, cur_force_mult, wall_force_mult);
                                
                                sysparams.bound_list = {};
                                
                                %                                MakeOctoberTrapezoidBoundaries
                                MakeFebTrapezoidBoundariesKrupicIsh
                            else
                                sysparams.folder_path = sprintf('../Outputs/FebTrapSetup/NonKrupic/Graining%d/Radius%fAspratio%f/RHWavevec%f/Angle%f/ArchOutputStrength%f/WallMult%f/',  sysparams.graining, landmark_cell_radius, sysparams.ellipse_asp_ratio, rh_wave_vec, sysparams.rh_rot_angle, cur_force_mult, wall_force_mult);
                                
                                sysparams.bound_list = {};
                                
                                MakeFebTrapezoidBoundaries
